...
| **FR-1225 — both sides of a card in one capture** | 5 Sep 2026 | **Pass, eight of nine rows, and the ninth found a defect.** Front then **Add another photo** then back: `chars` **141 → 310**, one classification over the concatenated lines, **three** phone numbers where the front alone gave one, the address assembled from the back's two lines, and the front's name and company kept. A job title edited before the second side survived **three** further re-classifications. Four photographs withdrew the control rather than discarding a fifth. Three blank sides gave **"1 of 4 photos read"**. A forced rotation returned `chars=310`, never 141 — the disk-enumeration reading of SRS 1.124 doing its job. `cache/camera` held two files, then four, and was **gone** each time the capture closed. **The account half**: a two-sided card saved and undone inside the window, *"Undone. Nothing was kept."*, device contact count 3174 before and after. **Owed**: the control's absence on a QR-decoded card, and `Save contact`'s `enabled` flag read in the same dump as the reading note rather than derived from it. |
...
| **The control is not offered on a decoded card** | Photograph a card carrying a contact QR: the sheet opens from the grammar and **Add another photo must be absent**. A vCard payload is complete, and merging recognised lines into it needs the conflict policy FR-1225 exists to avoid | a card with a contact QR |
