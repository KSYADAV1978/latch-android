...
| **FR-1232 — undo of an update** | — | **NOT RUN, no longer blocked.** SRS 1.165 is closed, so the control is reachable: the `Update` control cannot be reached on the photographed path. It remains the most consequential unverified row in this pillar — after the patch the previous values exist nowhere else, and an undo that deleted rather than restored would be data loss on a contact the user owned before Latch touched it. |
