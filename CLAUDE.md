...
| **FR-1201a — the camera, end to end to the sheet** | 5 Sep 2026 | **Pass, on the halves a device can settle, and it found two things.** The filled **Capture a business card** button opened Google Camera in Latch's own task with **no permission prompt**, and the installed APK requests no `CAMERA` at all — read off `dumpsys package`, so the B2 decision is asserted against the artifact rather than the source. Three photographs, three card sheets, **straight to the card** each time and never a dates sheet with a card button beside it. `content ready, ocr=true` at 416, 449 and 1019 ms against NFR-101's 2.5 s. **FR-1211 watched on both sides of the rule**: `cache/camera/card.jpg` present at 1,680,685 bytes during the capture, and after the sheet was closed the **directory itself is gone**, the cache back to the one `data` entry it held beforehand. **The killed-process half came free from a reinstall** — the process died with a sheet open, leaving exactly one file, and the next photograph replaced it. Two findings travel with it, both recorded rather than fixed in passing: SRS 1.122, the card sheet having **no background of its own** and surviving eleven earlier device rows because every one of them had a dimmed gallery behind it rather than Latch's own home screen; and SRS 1.123, the same card photographed twice producing **different recognitions and therefore different FR-1208 hashes**, which makes "Already saved" meaningless on this path. **Not established and not to be read into this**: the "Reading the card…" spinner, a rotation, a blank-wall capture, FR-1003's toggle, and `latch.card.layer` reading `CAMERA` in a real contact — that last needs a write and none was made. |
| **FR-1225 — both sides of a card in one capture** | 5 Sep 2026 | **Pass, all nine rows.** Front then **Add another photo** then back: `chars` **141 → 310**, one classification over the concatenated lines, **three** phone numbers where the front alone gave one, the address assembled from the back's two lines, and the front's name and company kept. A job title edited before the second side survived **three** further re-classifications. Four photographs withdrew the control rather than discarding a fifth. Three blank sides gave **"1 of 4 photos read"**. A forced rotation returned `chars=310`, never 141 — the disk-enumeration reading of SRS 1.124 doing its job. `cache/camera` held two files, then four, and was **gone** each time the capture closed. **The account half**: a two-sided card saved and undone inside the window, *"Undone. Nothing was kept."*, device contact count 3174 before and after. **The decoded case closed the set**: a generated card printing one identity and encoding another opened on the **encoded** one with **Add another photo absent** and `LatchCardOcr` silent, while the same card shot too far away to decode showed the printed identity **with** the control — the positive control for the same row. **Owed**: `Save contact`'s `enabled` flag read in the same dump as the reading note rather than derived from it. |
| **FR-1226 — the card as the contact's photo** | 6 Sep 2026 | **Pass on the mechanism, at the third attempt, and the two failures are the record.** The tick is off by default and the whole row toggles (Material's default left the label inert, which is wrong on a sheet this tight). Ticked and saved, the first run answered *"The contact was saved, but Google would not take the picture"* — FR-1226's own reading working, and the whole of what could be learned, because the failure had no name. A diagnostic added on the spot gave **`status=404 reason=null`**: `updateContactPhoto` is bound to **PATCH**, not POST, and Google answers an unrouted method+path with a 404 rather than a 405 (SRS 1.128). The same line exonerated the encoder — `43964 bytes` had already been produced, so decode, EXIF rotation, circle fit and JPEG encode all worked before the request was made. On PATCH: **`card photo attached, 44791 bytes`**, no failure note, and the undo that followed said *"Undone. Nothing was kept."* **Three contacts were created in the developer's account over the three runs and two of them remain** — the first two undos were mis-tapped by the operator against a stale layout dump, not refused by the app. Latch never deletes contacts, so they are the developer's to remove. **Two instrument corrections came out of it and both weaken something already written down** (SRS 1.129): `alreadyGone` swallows a 404, so "Undone" cannot by itself distinguish a delete Google accepted from one it never routed; and the device's `contacts` provider is the **aggregated** view, where two raw contacts sharing an email collapse into one row — `raw_contacts` showed two where the count showed one. Every count taken with the aggregated view in this record is a lower bound. |
...
| **FR-207 — the page counter, and NFR-101a re-measured** | 10 Sep 2026 | **Pass, and it corrected my own instrument.** `long.pdf` counted `Reading page 2 of 10…` → `5 of 10` → `10 of 10` — **"of 10", not "of 14"**, so the counter reports what will be read rather than what the file holds — then the sheet showed **"First 10 of 14 pages read."**, two different sentences about the same ten. `letter.pdf` counted `1 of 2` with the cap line correctly absent. An image capture produced **no** page counter at all across ~44 samples on two fixtures. **The timing had to be taken twice and the first figure was wrong.** Polled for the progress line, the read reported **7130 ms** against NFR-101a's 1.5 s + 500 ms/page = **6500 ms**, which reads as a breach; it is not. Continuous `uiautomator dump` is expensive and was running *through* the read. Unpolled, twice: **5128 ms and 5233 ms**, comfortably inside, and within 3% of the 5074 ms this record holds from 31 Aug. **Measuring the progress line and measuring the budget cannot be the same run** — the instrument for one destroys the other. |
| **FR-1005 — the `.ics` as a wire format** | 10 Sep 2026 | **Pass, on the bytes rather than on the screen.** An export writes to `cache/exports/` and the file was read back through `run-as`. An ordinary capture: **13 CRs and 13 LFs**, so every line ends CRLF and none is bare; `DTSTART;TZID=Asia/Kolkata:20271008T150000` with a one-hour `DTEND`; a fresh `UID`; and — the clause that matters — **no §7.2 metadata anywhere**, no `source_hash` in a file a user might email. **Exported twice, the UID changed** (`ab181b76…` → `62131d41…`), so two imports are two items and not one replacing the other, and the directory holds exactly one file because it is swept each time. **Nothing was written**: no `save decision=` line, `latch.db` untouched. **The Devanagari case is the one worth the trouble, and it is measured in octets.** `परियोजना समीक्षा बैठक कार्यालय में …` exported with **14 CRLF pairs, 0 bare LF**, a `SUMMARY` folded at **exactly 74 octets** with the continuation line beginning with a space, **no line over 75 octets and no broken UTF-8**. Folding by *characters* would have run well past 75 octets before wrapping, so 74 is the proof that the implementation counts octets — the exact thing SRS said a Devanagari title would expose. **Noted, not a defect**: the *filename* is derived from the ASCII part only (`on-8-October.ics`); the content is intact. |
| **AC-06 on Android, and the write halves of Slices 3 and 4** | 10 Sep 2026 | **Pass, eight rows, with every dated fixture deliberately placed in September 2027** so the developer can find and remove the lot in one month — the arrangement they asked for, and the reason nothing here was undone: not undoing removes the ten-second race that cost two unremovable events on 8 Sep. **AC-06 had only ever run on Windows.** Re-based to `Project sync on 14 September 2027 at 11:00` — Tue 14 Sept, three working days back, landing **Thu 9 Sept 2027** with FR-606 reading **"weekend or rest days skipped (2 days)"**, the count corroborating the date independently. **FR-507's override reaches Google** on a single capture, on a range (dropping to the first day, as §8.1 warns before the tap) and **per row in a chain** — one event and one task from one message. **FR-510 per candidate**: `12 Mar 2026` — the current year, never rolled forward — with the past note and the structurally-fixed badge, beside `20 Sept 2027` with neither. **`basis=KEY_NOT_CONSULTED` was seen firing** on every multi-item save, which is SRS 1.25's narrowing observed rather than reasoned about: for a chain the `item_key` query is skipped outright, and *nothing was looked for* is a different fact about the account from *nothing matched*. **FR-803 over a chain answered `basis=SOURCE_HASH`** — the strong query, not a fall-through — and wrote nothing. **One near-miss worth recording as method**: the recipe chooser renders as a bare label with no chips until it is tapped, which looked exactly like a defect and is deliberate — the chips are folded away because nine of them at a large font scale push the sheet off the glass, and the code says so. Reading it took a minute and would have cost a false defect report. |
| **FR-605 — a preference that changes the arithmetic** | 10 Sep 2026 | **Pass on both halves, and nothing was written to the account.** The AC-06 fixture was re-run against three different working-day configurations and the *step moved each time*. Five-day week: prep on **Thu 9 Sept 2027**, "weekend or rest days skipped (2 days)". **Saturday ticked**: prep on **Fri 10 Sept 2027**, and the line became **"weekend or rest day skipped (1 day)"** — the **singular** branch of that string, rendering on Android for the first time. **A holiday added on the prep day**: prep on **Thu 8 Sept 2027**, with a *second* line beside the count — **"Skipped: LatchTestHoliday"** — so the row names the holiday rather than merely stepping over it. **Both settings were restored and the restoration verified by behaviour rather than by the screen**, which mattered: the first removal tap missed, the holiday list said nothing about it, and only re-running the capture showed the step still sitting on the 8th. **The count corroborates the date independently in every configuration** — 2, then 1, then 2-plus-a-named-holiday — which is what makes this stronger than reading a preference back. **The chips report `selected=false` throughout**: Compose state does not reach that attribute, so the arithmetic is the instrument (SRS 1.188, a third time). |
| **FR-807 across a rotation and a process death, and FR-904's picker** | 10 Sep 2026 | **Pass, and both undos succeeded, so nothing was left in the account.** Saved, rotated to landscape: the offer was **still standing** — `Saved to Latch | Undo (5)` — where before SRS 1.44's fix a recreation silently ended it. Undone: `deleted=1 failed=0`. Then saved and **`am force-stop`ped two seconds later**: the home screen came back carrying *"Just saved. You can still undo it." / Undo (6)*, which is the limit FR-807's own note called unfixable without FR-701, and **the undo taken from the home screen** logged `save decision=Undone deleted=1 restored=0 dropped=0 failed=0` — closing SRS 1.193's last owed row, the path where an offer has outlived the process that made it. **FR-904's picker**: *Change* expands the list inline without blocking the sheet, all three writable calendars are offered, and choosing one closes the list and moves the chip. **The instrument lesson of the day, and it nearly cost a false defect report for the third time.** The picker appeared inert across a dozen taps — chips `clickable=true`, coordinates inside their bounds, handler that would have closed the list. It was not inert: the phone had **disconnected mid-run**, `adb pull` was failing silently and every dump was the same stale file. **The positive control is what caught it** — the badge would not flip either, and a control that fails alongside the thing under test means the instrument is dead, not the app. Every check after that was run control-first. |
| **FR-806's queue offline, and an FR-806b defect found on the way** | 10 Sep 2026 | **Pass on both queue rows, and the run found SRS 1.198.** Aeroplane mode with DNS confirmed failing first: a capture reported *"No connection. Saved on this phone…"* — never "Saved" — and left **one** queue entry. The home screen showed *1 capture waiting to be saved to Google* with **Retry now**, and **tapping it while still offline left the entry queued and lost nothing**, which is the half that makes it a manual retry rather than a button that only works when it was going to work anyway. On reconnection the drain ran within about 35 seconds — `Worker result SUCCESS for WriteQueueWorker`, queue back to `<map />`. **The defect is beside it**: the same home screen also carried FR-806b's *"Latch needs you to sign in to Google again"* over a **live** grant, and it **vanished when the network returned**. SRS 1.198 has the diagnosis; the short of it is that offline `authorize()` succeeds with a resolution rather than failing, so `GrantCheck.Unknown` — which exists for precisely this and says so — is never reached. **Also corrected here: the claim that `adb` was connected over Wi-Fi.** It is USB — `persist.sys.usb.config=adb`, no TCP adb port, and the link survived both Wi-Fi being disabled and aeroplane mode this time. The earlier disconnect that prompted the inference was **coincidence**, and the disconnections through the day remain unexplained. |
| **SRS 1.199 — the offline banner, fixed and re-watched** | 10 Sep 2026 | **Pass, under the conditions that produced the defect an hour earlier.** The fix installed and confirmed on the phone rather than assumed — the installed `base.apk` md5 equals the built artifact, and all six stores survived the upgrade. Aeroplane mode with `ping` answering **`unknown host`** first, so the fixture is the real one; a capture reported *No connection…* and left one queue entry; and the home screen then showed *1 capture waiting to be saved to Google* with **Retry now** and **no sign-in banner**. The same configuration on the pre-fix build had said *"Latch needs you to sign in to Google again"* over a live grant. Back online, the queue drained and the banner stayed absent. **The positive case was then closed the same day, and it was the only risk the fix carried.** The grant was revoked for real, and — the step September's attempt skipped — **the fixture was confirmed ready before anything was judged**: `PROBE_DEDUP` answered `SignInRequiredException: Authorization needs consent and nothing can present it`, with Play services' own `Auth: getToken() -> NEED_REMOTE_CONSENT` naming all five scopes beside it. Online, the home screen then showed the banner and its **Sign in** button. **A fix that only silenced the false alarm would look identical to one that silenced everything; this is the run that tells them apart.** **A captive portal remains a known residual**: it looks connected and is not, so it can still raise the false banner. |
| **The grant restored, and SRS 1.37's paging seen directly at last** | 10 Sep 2026 | **Pass on the restore, and an incidental observation worth more than it.** After signing in again: `PROBE_DEDUP` answered **`status=200`** on both queries where minutes earlier it had thrown `SignInRequiredException`, the home-screen banner **cleared**, and the queue was empty. **The observation is in the probe's own output**: `hash maxResults=1 -> status=200 items=0 nextPageToken=YES` beside `hash maxResults=250 -> status=200 items=0 nextPageToken=no`. **That is the empty first page carrying a token that SRS 1.37 named as the thing which would confirm the paging diagnosis**, and this record has been carrying it as the *leading hypothesis* — testable, and never actually seen. The 1 Sep probe reported `items=0` for both and said nothing about a token. So a one-event page really does come back empty with more of the calendar unscanned, while 250 covers it in one go and says so by returning no token; a query that stopped at the first page would answer "not found" with a match possibly waiting on the next. **It changes nothing in the code** — `findEventPaged` already follows the token to exhaustion, which was the right fix rather than raising the number — but the hypothesis is now an observation, and the sentence in this file that calls it leading can be read as settled. |
| **The Windows tray over a revoked grant** (SRS 1.200) | 10 Sep 2026 | **Observed, not a pass — a defect recorded.** With the grant genuinely revoked and Android's banner correctly raised, the desktop tray still read *Signed in as <the account>*. `secrets.dat` was unwritten since the previous day, so no fresh token existed and the stored one was dead. `isSignedIn` is presence, not validity. **This is SRS 1.177's mirror**: there the token had been *removed*, so the tray said *not signed in* and was accidentally right; here it is present and worthless and the tray is wrong in the direction that matters. **Mitigated rather than dangerous** — SRS 1.192 means the next capture is held with the right sentence rather than dropped, so the correction arrives at the first capture. Not fixed: making the tray honest costs a network call on a surface that reads four local files and opens instantly, which is a decision. |
| **SRS 1.192 and 1.195 in the wild** (SRS 1.201) | 10 Sep 2026 | **Pass, on a genuinely revoked grant, during a real recovery rather than a test.** The grant had been revoked to close FR-806b's positive case and the desktop was left holding a dead refresh token. A capture was **held** — *"Held on this machine. Sign in to Google from the Latch tray icon…"* — and the store carries every clause: `queue.dat` with one entry at `needs_sign_in: True` and **`attempts: 0`**, so the flag was set at **enqueue by the saver** and not by a failed drain, which is the choice SRS 1.192 argued for; and `secrets.dat` losing `google.refresh_token` in the same write, because the `invalid_grant` handler discards a token it knows is worthless. **That removal cleared SRS 1.200's dead end** and restored *both* sign-in rows. After consent at 18:35:50 the queue was **gone by 18:36:17** — `DrainTrigger.SIGNED_IN` (SRS 1.195) writing the held capture within half a minute instead of leaving it on the backoff. **Stronger than the run that closed the row**: 9 Sep used a forged invalid token; this is Google refusing a grant the user revoked, on a client nobody was testing. |
| **The account, left clean** | 10 Sep 2026 | **Every fixture this session wrote was removed by the developer by hand**: 5 calendar events and 7 tasks, all placed in **September 2027** on purpose so the whole set was findable in one month, plus one undated FR-510 follow-up that by its nature could not be. **The convention is worth keeping**: a write-testing session that agrees a single month up front costs the developer one search instead of an audit, and it is what made an eight-fixture run acceptable at all. Latch deleted none of it — the app removes only what its own undo removes, and everything here outlived its window deliberately, because not undoing is what removed the ten-second race that cost two unremovable events on 8 Sep. |
| **SRS 1.202 and 1.203/1.204 — the two decisions, watched** | 11 Sep 2026 | **Both pass, and the Android half found a regression in its own fix.** **Windows (SRS 1.202)**: reinstalled, and two things ruled out before the menu was read — the process started eight seconds after the jar was written, and `Sign in again` was confirmed inside `DesktopStrings.class` in the running jar, because `trayMenu` being a correct pure function says nothing about Swing drawing it and five defects here have had that shape. The row sits between `Settings…` and `Sign out`, the account line above it still does nothing, and pressing it opened consent **without signing the user out on the way**. **The pass is the store**: `secrets.dat` rewritten at 10:18:22 against a baseline of 18:35:50 the day before, all four records intact — the tray would read *Signed in as …* either way, which is SRS 1.200's trap used as the instrument. **Android (SRS 1.203)**: the top-right *Done* now saves, watched with the saving button below the fold so the discarding one was the only *Done* on screen — the exact trap. **And the row's own "also worth one look" line caught SRS 1.204**: collapsing an *untouched* card marked the built-in as edited, because FR-603 stores an edit as a copy that **shadows** the original, so an identical value still made the shadow. 1.203 had claimed that cost was zero. Fixed by saving only where `draft != recipe`, and both halves re-watched. **Recipes left exactly as found** — the shipped eight, no markers. |
| **The upgrade, and the launch canary** | 6 Sep 2026 | **Pass.** `adb install -r` over the 12:27 build. All six stores survived — `account_defaults.xml`, `card_queue.xml`, `secrets.xml`, `settings.xml`, `write_queue.xml` and `latch.db` — so setup did not run again and nothing was wiped. `MainActivity` reached `topResumedActivity`, no `FATAL EXCEPTION`. |
| **SRS 1.149 — the four-section home screen** | 6 Sep 2026 | **FAILED, fixed, and re-verified — see SRS 1.158.** The four sections rendered correctly with their headings and notes, and the dates note no longer read as a caption for the card button, which was the defect 1.149 fixed. But the `Column` had **no `verticalScroll`** — the only screen in the app without one — so on a 1440x3120 Pixel the last node ended at **y=2909** and **Settings was off the glass**. Two hard flings moved nothing and `uiautomator` reported no Settings node at all. FR-1000 was unreachable, and with it FR-1003's toggles, FR-905's routing, FR-1004's webhook, NFR-205's disconnect and FR-209's disclosure. After the one-line fix: the screen scrolls, the headline moves off the top, Settings appears with heading, note and button, and **tapping it opens the Settings screen** — destination calendar, task list and routing all present. |
...
| **Slice 3's confirmation-sheet rows** | 6 Sep 2026 | **Pass, all read-only — nothing was saved.** FR-506 row 3: *call at 4pm* showed **Today / Tomorrow / In a week / Other date…** without a tap, nothing pre-selected, routed to the Inbox; tapping *Tomorrow* filled `7 Sept 2026, 4:00 pm`, turned the button to **Save**, and made the recipe chooser appear — the blocker re-asked on an edit, which is the Android twin of a Windows row. AC-04: *the order dated 12 March* → **TASK 12 Mar 2026**, the missing year resolved to *this* year and never rolled forward, with FR-510's follow-up note and *A past date cannot be an event*. FR-510 per candidate: two rows, both ticked, the past note on the first only and the recipe chooser correctly absent with its reason. FR-507: the badge flips both ways. A range overridden to a to-do: the cost note names the loss **before** the tap and the to-do falls on the first day. |
| **The action row fits a tall capture** | 6 Sep 2026 | **Pass.** A five-date capture: all five ticked, the scrolling region exactly **1820 px** — `SHEET_CONTENT_MAX` at this density — and the action row wrapped to two lines with Close, Export .ics and Save all inside the window. This is the row the FR-804 offer's clipped *Up…* label opened, and it confirms the bound works on `CaptureScreen`. It also narrows SRS 1.165: the same constant holds here and on the card sheet from **text**, and fails only on the card sheet from a **photograph**. |
| **FR-504 through Settings** | 6 Sep 2026 | **Pass, with the reading corroborated rather than just the string.** `Invoice 05/10` read **5 Oct 2026** day-first; switching *Reading 05/09* to **9 May** made the same capture read **10 May 2026** — and carry FR-510's past-date note, since 10 May is past and 5 Oct is not. The note is what makes this more than a formatting change: the *interpretation* moved, not the rendering. The stored record's ciphertext changed too, so the preference was written; unlike the Windows twin (SRS 1.80) it cannot be read in plaintext, because NFR-204 encrypts it. **Restored to day-first afterwards and verified.** |
| **FR-512's threshold through Settings** | 6 Sep 2026 | **Pass, and it is the strongest single check of the Settings screen.** The same capture — `Kickoff 8 September 2027 at 9am` — offered **Save** at 60 and **Add to Inbox** at 99, with *Latch is not confident about this one, so it goes to your Inbox to check first.* A preference stored and then actually **read by the capture path**, which is what the Windows row of 4 Sep called the same thing. **Restored to 60 afterwards and verified.** |
| **FR-1003's layer toggles** | 6 Sep 2026 | **Pass on all three, and the tile against the package manager rather than the screen.** Turning the Quick Settings tile off put `com.latch.android.capture.CaptureTileService` into `disabledComponents` and `cmd package query-services -a android.service.quicksettings.action.QS_TILE` stopped returning it — FR-1003's *removes it from Quick Settings*, verified where it takes effect. Turning it back on restored both. The share sheet switched off leaves Latch **still registered** as a `text/plain` target, which is the documented Android limit rather than a defect — three filters on one activity — and the capture declines with *This way of capturing is switched off in Latch's settings*. **It also found SRS 1.171**: that refused capture still offered Save and Export .ics, both of which did nothing. Both toggles were restored and the restore verified. |
| **NFR-203's endpoint masking, and FR-1004's two acts** | 7 Sep 2026 | **Pass on the screen *and* at rest, which is the pair that matters.** An endpoint with a token in its path saved as **`Saved: https://127.0.0.1/••••••••`** and the path appeared nowhere on screen; `secrets.xml` held one base64 ciphertext under `webhook.endpoint` with **neither the path, the host, nor even the scheme** in the clear. A mask alone would not have shown the second. It survived a cold start still masked. A plaintext `http://` endpoint is refused with the right reason — *Only https addresses are accepted — your captures would otherwise travel in the clear* — and nothing is stored. Removing it also cleared `webhook.last_delivery`, which is FR-1004b's *the report does not follow a removed endpoint*. **Two defects found**: the enable switch refused silently (SRS 1.172) and removal left a phantom `Saved: ••••••••` after a restart that re-enabled the switch with nowhere to send (SRS 1.173). |
| **AC-03, FR-701 to FR-704 — the Inbox, on the real storage** | 7 Sep 2026 | **Pass, and it is the first exercise of `:data`'s SQLite by hand rather than by the instrumented suite.** AC-03: *Ask about the uniform order* opened as **TASK / No date found** with *This will be saved without a date. Latch never guesses one*, the button read **Add to Inbox**, and pressing it answered *Added to your Inbox on this phone. **Nothing was written to Google yet.*** — `latch.db`'s mtime moved from 4 Sep, so the row reached the database. **FR-704**: the home screen then read *1 capture waiting in your Inbox*; after the row was discarded it showed **no count and no Inbox control at all**, which is the requirement's *shall not nag* at zero. **FR-703 is stated on the screen itself** — *Everything here is on this phone only. Nothing reaches Google until you save it.* **FR-702's actions, each verified across a `force-stop`**: a date assigned through the picker (`TASK 14 Sept 2026`), a title edited, and the row discarded — all three survived a process death, which is the row the instrumented suite cannot reach because it writes and reads inside one process. **Design principle 1 held in the picker**: with nothing chosen, *Use this date* is inert — tapped, the dialog stayed open — so the app never picks a date on the user's behalf. **Not tested and not claimed**: *Save to Google* (it writes) and *Snooze a week* (it would leave state behind). **One observation, not a defect**: a row that has been given a date still shows its original reason, *No date found… give it a date there*. The reason is correct as provenance (SRS 1.18's reading) but its instruction is stale once acted on. |
| **Slice 3 — FR-506 row 3, FR-507, FR-510, and AC-04's sheet half** | 7 Sep 2026 | **Pass, eleven rows, and not one of them needed a write** — every decision these three requirements take is visible on the sheet, so the account was untouched throughout. **FR-506 row 3**: `call at 4pm` opened with *Pick a day for this one:* and **Today / Tomorrow / In a week / Other date… already on screen**, no tap needed, under *There is a time here but no day. This goes to your Inbox*, with the button reading **Add to Inbox**. Pressing *Tomorrow* completed the row to `8 Sept 2026, 4:00 pm`, took the Inbox line away, brought up the destination chip and changed the button to **Save**, `enabled=true` — and the **recipe chooser appeared on the same edit**, so the blocker is re-asked rather than computed once, which is the Android half of a row Windows closed on 4 Sep. *(There is no checkbox to tick, as the backlog row expects: a single-candidate sheet renders none.)* **`Other date…`** opened the M3 calendar over the sheet with `Current selection: None`; picking **Sunday 20 September 2026** on an `Asia/Kolkata` device landed **`20 Sept 2026`**, not the 19th or the 21st, so the picker's UTC-midnight millis are converted through the device zone correctly. **FR-507**: the badge flipped **EVENT → TASK → EVENT** on one row, the when-line following it (`8 Sept 2026, 4:00 pm` ↔ `8 Sept 2026`) — and **the time came back**, so an override withholds a time from the display and the write rather than destroying it. **The cost is shown before the tap and is specific to what would be lost**: *keeps the day but not the time* on a timed capture, *keeps the first day but not the last — a to-do has one date, not a span* on `Trip from 20 September to 24 September 2027`, which then dropped to `20 Sept 2027` when flipped, exactly as promised. That capture also showed **SRS 1.168's range span on a device for the first time**: `20 Sept 2027 – 24 Sept 2027`, both ends. **AC-04's sheet half**: `the order dated 12 March` resolved to **12 Mar 2026** — the current year, never rolled forward — with FR-510's line and *A past date cannot be an event.*, and **the badge there is a `TextView` with `clickable="false"`**, so it is structurally not flippable rather than a button that refuses. **FR-510 per candidate**: `Invoice dated 12 March, payment due 20 September 2027` gave two ticked rows, the past one carrying the fixed badge and the future one a full-size flippable button, plus the multi-date reason *A recipe expands one date, and this capture has several.* **The action row fits** on the tallest of these — chips, badge, cost note and three actions inside a window ending at y2660 with the last button at y2553. **What is NOT established**: every "reaches Google" half of this slice — an override written as a task, a chain writing one event and one task, AC-04's undated follow-up in Google Tasks, and a queued follow-up keeping its note. Those need writes and none was made. |
| **FR-507 on an Inbox row, and it is stored** | 7 Sep 2026 | **Pass, and stronger than the row asked for.** A row given a date in the Inbox flipped **TASK → EVENT** on its badge, which is FR-507's *"before saving"* reading — a row is saved from there. Then `am force-stop`: the row came back reading **EVENT**, so the override is written to `:data`'s SQLite rather than held in the screen. A row's assigned date, its edited title and now its type override have each been watched across a process death, which is the half the instrumented suite cannot reach. **The save half is owed** — the item Google receives has not been checked against the flipped badge. |
| **The home screen's four sections** (SRS 1.149) | 7 Sep 2026 | **Watched, and it had not been.** Business cards, Dates and to-dos, Recipes, Settings, each with its note. The dates section has **no capture button** and says why — *There is nothing to press here — you start from the app you are reading.* FR-704's count sits **inside** that section rather than above the sections with the alerts, which reading the code settled rather than the screen: the count belongs to a pillar and is a standing fact about it, where FR-807's offer and the sign-in banner are things that happened. |
...
| ~~**FR-1231's `Update`, on a photographed card**~~ | 7 Sep 2026 | **PASS, and SRS 1.165 is closed** (SRS 1.182, 1.185). The cause was one line of nesting: the `when (saveResult)` block was a **sibling** of the scrolling column, so an offer's rows were laid out unbounded beneath a bounded column, ran off the window and took the action row with them — `Update` was never scrolled off, it was **never composed on screen**. 1.165's own ruled-out table said a brace walk put those rows *inside* the scroll, and that false exoneration is what cost two days. **Watched on the camera path with an offer standing**: window off the status-bar ceiling at `[160,354][1280,2827]`, scroll region **1820 px `scrollable="true"`** where there had been none, and `Close` / `Save as new` / `Update` all present and `enabled="true"`, `Update` ending 107 px clear of the window bottom. `Save contact` correctly absent — FR-1231 withdraws Save while the offer stands. **Nothing was written.** **What broke it open was a reproduction needing no camera and no write**: FR-1230 reaches the same sheet from a text selection and FR-1231 keys on an email identity, so a generated card differing from an existing contact raises the offer on demand in about forty seconds — where every earlier attempt had been graded on one photograph taken by hand. |
| ~~**FR-1231's offer across a rotation**~~ | 7 Sep 2026 | **PASS** (SRS 1.183, 1.184). Reported from real use: a rotation dismissed the offer and returned the sheet to **Save contact** — the control that writes a *second* contact for somebody the offer had just said is already there. The offer lived in the composition's `remember`; it is now held at application scope and keyed on `captureKeyOf`, which is FR-807's own arrangement one API over. Rotated to landscape and back: `Close` / `Update` / `Save as new` **present** and `Save contact` / `Add another photo` **absent** — the exact inverse of the failure — with **four checkboxes, all four still ticked**, so the user's replies survive and not merely the question. `Update` was present in landscape too. |
| ~~**FR-1226's tick on the update path**~~ | 7 Sep 2026 | **PASS** (SRS 1.186-1.188). Reported from real use: the picture was ticked, the update landed, and the contact's photo did not change. `update()` takes no photograph at all and `createAnyway`'s call site passes none either, so the tick was inert on **both** answers while reporting success. **Not wired up, deliberately**: `ContactRecord` has no photo field, so a replaced picture could not be restored and the change would be **unundoable** — FR-1232's own harm, reached from a new direction. The tick is off while an offer stands, with the reason beside it. **Watched on the camera path with `UpdateOffered changes=3`**: the sentence is there, the tap does nothing (`checked` false before and after), the offer's own three ticks still toggle and were left as found, and the pre-offer baseline on the same card had the tick live and no sentence. **`uiautomator` called the control `enabled="true"` throughout** — it reads the merged `toggleable`, whose bounds span the row, and Compose's disabled state never reaches that attribute. The press is the instrument. |
| ~~**FR-1232 — undo of an update**~~ | 7 Sep 2026 | **PASS, and it was the last unverified row in this pillar** (SRS 1.189). An offer of **three** changes; the website and address unticked by hand, the job title left; accepted, then undone inside the ten seconds — and **Google Contacts shows the title the contact held before the update**, with the contact still there. A restore, never a delete, on a contact the user owned before Latch touched it and whose prior value after the patch existed nowhere but in memory on the phone. **`card decision=Updated fields=1` against an offer of three** is the half worth more than the pass: `acceptedChanges` sent what was ticked rather than what was offered, which is SRS 1.159's per-field ticks seen reaching the wire for the first time. **Owed:** the undo logs no decision line of its own, so its evidence is the account rather than the phone — the gap SRS 1.72 cured on the date side with `WriteBasis` and which the undo path still has. FR-1207's `captured_at` was not read back, so write-once across an update stays JVM-only. |
...
somebody. It did not happen: the selector looked for `Export .ics`, did not find it, and the
script exited **without tapping**, which is the guard `share-to-latch.ps1` already encodes
generalised to every UI script here. **Dismiss that chooser with `KEYCODE_BACK` and never with a
tap**, and read the exported file from `cache/exports/` instead of judging the export by what the
chooser shows.
...
**The leading hypothesis is paging, and it is testable — and on 10 Sep 2026 it was finally seen.** `PROBE_DEDUP` returned `maxResults=1 -> items=0 nextPageToken=YES` beside `maxResults=250 -> items=0 nextPageToken=no`: the empty first page carrying a token, which the paragraph below names as the confirming observation. It is no longer a hypothesis.

**The original reasoning, kept because the method in it outlives the bug.** `eventDedupUrl` asks for
...
| ~~**The menu dismisses**~~ | **FAILED in real use then PASSED on the fix, 9 Sep 2026 (SRS 1.196), on both halves.** The invoker was a `JWindow`, which `Window.isFocusableWindow()` refuses twice over, so `requestFocus()` was inert and `windowLostFocus` could never fire. As a `JDialog`: **clicking away closes the menu and Esc closes it**. Esc had never been pressed on this client before today — it failed for the same reason and was fixed by the same one-word change | done |
...
| ~~**FR-1202 NONE — a text capture**~~ | **PASS, 4 Sep 2026.** No card action anywhere on an ordinary dated text capture. |
| ~~**FR-1202 AVAILABLE — an image with no code**~~ | **PASS, 4 Sep 2026, at the second attempt — and the first attempt is the lesson.** `three-dates-v4.png` offers the quiet button, and tapping it now says *no contact code in this image*. The **first** pass was ambiguous: while a swallowed decode looked identical to an honest "no code here", a green AVAILABLE proved nothing. Re-run after SRS 1.100's fix. |
| ~~**FR-1202 DETECTED — a card QR**~~ | **PASS, 4 Sep 2026, after two defects.** `card-anita.png` promotes the action to a filled button, first in the row, and it opens the preview. It failed first because the decode raced the recognition and because the tap declined silently — SRS 1.100. |
| ~~**FR-1203 — two codes, and the refusal to guess**~~ | **PASS, 4 Sep 2026.** `card-two.png` opens the chooser, numbered and unpreviewed; neither sheet opens by itself. **The fixture found a third defect before the row ran**: the single-result reader could never have produced two payloads, so this row was unreachable until `decodeMultiple`. |
| ~~**FR-1205 / FR-1213 — the preview**~~ | **PASS, 4 Sep 2026, at the second attempt — and the first attempt found the worst defect of the pass.** Every field is a text box already; the account line and the never-deletes line are present; blanking the last identifying field disables Save with a reason and typing one back re-enables it. **The first attempt failed and wrote a contact**: `applyTo` left `givenName`/`familyName` untouched, so blanking the one Name field the sheet shows cleared nothing and the account received a name the user had removed (SRS 1.101). The contact was found by its FR-1207 record — not by name, which was the field that had been cleared — deleted, and the account verified empty of card records. |
| **Dismissing the card sheet** (SRS 1.104, 1.147) | **FAILED in real use, 6 Sep 2026, and fixed — the second half is still owed.** Reported after a camera capture: saving the contact and closing brought back a capture sheet reading *no date found* with *Save as contact* beside it, which is this row's own failure sentence. The rule was right and its input was wrong — `candidates.size` counts the single `TASK_UNDATED` that a capture with no date in it always yields, so no card could ever reach the closing branch. `unsavedDates` counts `date != null` now, pinned by a test over the real parser and confirmed red against the old expression. **Still owed on a device**: the fix itself, and the other half — a capture holding both a code and a date, where dismissing after the contact must return to the **dates** | any card; and a flyer with a date and a code |
| ~~**FR-1212 — a card held offline reaches the account**~~ | **PASS, 4 Sep 2026.** Offline save said *"held on your phone"* — never "saved" — and the record was on disk, 1,161 bytes, with **zero** occurrences of the card's text in the clear. On reconnection the drain **wrote** it, and the record carried the instant it was **captured** rather than the instant it drained. |
| ~~**FR-1212 — a held card survives a restart**~~ | **PASS, 4 Sep 2026.** Still on disk after `am force-stop`, and drained afterwards. The half the record tests cannot show: they write and read inside one process. |
| ~~**FR-1212 — the check runs again at drain**~~ | **PASS, 4 Sep 2026 — the row that matters.** A card already in the account, captured again offline, **retired without writing**: one contact afterwards and the **same resource id**, not a second. AC-07 failing inside AC-10 on a third transport — the sequence the calendar failed for five days (SRS 1.37). **The contrast is the evidence**: the same path with a card the account did not hold wrote it. |
| ~~**FR-1210 — undo of a held card deletes nothing**~~ | **PASS, 4 Sep 2026, at the second attempt — the first found that the undo had no control to press.** The entry left the disk and **nothing arrived on reconnection**, which is the half that matters: a delete for a contact never created succeeds against a 404 and looks exactly like success. |
| ~~**5a — the write**~~ | **BUILT and WATCHED 4 Sep 2026 (SRS 1.98, 1.100–1.103).** All eleven device rows pass. Four defects were found by running them — the single-result QR reader, the decode racing the recognition, a card action that declined silently, and `applyTo` leaving the structured name behind so a cleared name still reached the account. One row was wrong rather than failing. | **Eleven rows, all struck** |
| ~~**FR-1208 — the same card twice writes once**~~ | **PASS, 4 Sep 2026, on the count.** `card-anita.png` saved, saved again, *"Already saved. Nothing was written again."* — and the account carries **exactly one** contact with a card record, read back through the app's own client. The sentence is not the evidence; SRS 1.38 records it arriving from the wrong query. |
| ~~**FR-1207 — the record is on the contact**~~ | **PASS, 4 Sep 2026.** Read off a real contact: `latch.card.version`, `source_hash`, `identity`, `captured_at`, `layer`. Both identity fields are digests rather than addresses. |
| ~~**FR-1209 — a card with no email writes no identity**~~ | **PASS, 4 Sep 2026**, on `card-noemail.png` — a card that genuinely carries no email, which is what many printed cards are. The record has **no `latch.card.identity` key at all**, absent rather than empty, beside Anita's which has one. *(The row originally said "a cleared address": editing the sheet cannot change the duplicate identity, because FR-1208 hashes the payload and not the draft, so a re-capture would only ever answer "Already saved".)* |
| ~~**FR-1205 — the correction is what is written**~~ | **PASS, 4 Sep 2026.** A job title edited to `Managing Partner` on the sheet is what Google holds — not the parsed `Partner`. |
| ~~**A blank field is absent, not empty**~~ | **The row was wrong and the code was right (SRS 1.102).** It asked that clearing the company leave no organisation at all; the account showed an organization with an empty name, which is **the only representation the People API offers** — a job title is a field *inside* `organizations`. Restated as a JVM test: a title with no company sends an organization with a title and **no `name` key**, and clearing *both* omits `organizations` entirely. |
| ~~**A failed search writes nothing**~~ | **PASS, 4 Sep 2026**, with the device confirmed unable to resolve `people.googleapis.com` rather than merely showing an aeroplane icon. The save reported *"Could not save that. Nothing was written"* and the account held **three contacts before and three after**. The wrong answer to watch for was the reassuring one: *"Already saved"* would have meant a search that threw being read as one that found nothing. **The counting probe then reported a false zero** over that same account, having broken out of its scan while the phone was still reconnecting — `scanCapped`'s rule applied to production code and not to the instrument (SRS 1.103). |
...
discarded the pixels, which is what SRS 1.133 was reasoning about and why it answered "no". A crop
applied to the **source**, re-decoding the region with `BitmapRegionDecoder` at `sample=1`, recovers
all of them: a card at **half** the frame reads at **41 px of glyph, the same as one filling it**;
at a third, 27 px against 14; at a quarter, 21 px against 10. 2–10 MB against a 32 MB budget, and
no dependency. **Crop to the recognised text, not the card's edges** — the app already holds a
bounding box per block after pass one, and their union is indifferent to background, card colour,
whether the card is rectangular, and aspect ratio. Edge detection would buy none of that and costs
either ML Kit's Document Scanner (new dependency, Play services, network on first use, an
...
the answer and is not: **2,273 names in this account appear more than once** in it, the account
having several sync sources, so one created contact routinely shows as two rows — and a reading
taken between the two is mid-sync and means nothing. **Google Contacts on the web is the only
authority.** Every device-side count in this file is an indication, not a number.

**Framing dominates a card capture, and the decode is why** (SRS 1.133). `inSampleSize` takes only
powers of two, so a 12.2 MP photograph is decoded at **half on each axis** — 2016 x 1512 — and the
card's text is read from whatever share of 2016 px the card occupies: **41 px of glyph filling the
frame, 20 px at half, 13 px at a third, 10 px at a quarter**, against an ML Kit floor of roughly
16–20. A card at arm's length is not slightly worse, it is over a cliff. **Cropping and deskewing
are not owed** — ML Kit already handles in-plane rotation, a document scanner is a new dependency
and an interactive UI, and no crop recovers resolution the decode has already thrown away.
**`RGB_565` would**: the full 4032 x 3024 fits in 24 MB against the same 32 MB budget, four times
the pixels, 27 px of glyph at a third of frame. **Recorded and deliberately not changed**, because
it touches every capture and an OCR change is measured on a device here rather than reasoned
...
| **SRS 1.193 — `restored`** | **PASS, 9 Sep 2026 (SRS 1.194), and FR-1232 is now closed on the phone as well as on the account.** `card decision=Undone deleted=0 restored=1 dropped=0 failed=0`, after `UpdateOffered changes=1` and `Updated fields=1 mask=organizations`. **The fixture was a contact the developer created by hand in Google Contacts**, so Latch called `createContact` not at all and the only write in the run was the patch the undo reversed — which tests FR-1232's actual sentence, *the contact was the user's before Latch touched it*, rather than the weaker claim a Latch-made fixture would have tested. **Confirmed in Google Contacts on the web**, the only authority for a contact (SRS 1.135): the job title is back at `Head of Widgets` and the contact is still there. **8.3 seconds** from `Updated` to `Undone`. |
...
undoing* — arriving from a new direction: it is tight for an *instrument* too. **The number is
8.3 seconds** (SRS 1.194), measured on the run a person drove: `Updated` to `Undone`, 1.7 s of
margin, and nobody stopped to look at the account on the way. **The second was a
...
| Check | What failure looks like | Fixture |
|---|---|---|
| ~~**SRS 1.192 — a held capture asks for a sign-in**~~ **PASS 9 Sep 2026.** Verified on the store, not the screen: `queue.dat` present with one entry, `needs_sign_in: True` at `attempts: 0` — so it was set at **enqueue** and not by a failed drain — and `google.refresh_token` **gone** from `secrets.dat`, which proves the refresh reached Google and came back `invalid_grant` rather than the weaker not-signed-in branch. Original row: | Revoke or expire the grant, then capture on Windows. The window must say *Held on this machine. Sign in to Google…* and **never "No connection"** — that sentence over a live network names the wrong cure. `queue.dat` must exist and hold the entry. Failure is the 7 Sep behaviour: a refusal on screen and no file at all | revoke at `myaccount.google.com/connections`, or wait out the grant |
| ~~**SRS 1.192 — the tray asks, and pressing it signs in**~~ **PASS 9 Sep 2026**: the row read *1 waiting - sign in to save them*, and pressing it opened the browser. Original row: | The count row reads **"N waiting - sign in to save them"**, not *waiting to be written*, and pressing it opens the browser rather than retrying. Failure is the ordinary sentence, which names a wait the user can do nothing about | as above, with something queued |
| ~~**SRS 1.192 — signing in drains it**~~ **PASS 9 Sep 2026**, within seconds — `queue.dat` gone, a fresh `google.refresh_token` written. **It also exposed SRS 1.195**: the drain had no mutual exclusion and two of them wrote the capture twice. Original row: | Sign in from that row: the entry writes **within seconds**, not at the next backoff. Failure is nothing happening for up to half an hour after the user did exactly what was asked, which reads as the request having been pointless | as above |
| ~~**SRS 1.193 — the undo says what it was**~~ **PASS 9 Sep 2026 (SRS 1.194)**: `card decision=Undone deleted=0 restored=1 dropped=0 failed=0`, with `mask=organizations` and no `clientData` on the update before it. Original row: | `adb logcat -v time \| grep LatchTiming` across a card update and its undo: `card decision=Updated fields=N mask=…` with **no `clientData`** in the mask, then `card decision=Undone deleted=0 restored=1 dropped=0 failed=0`. Failure is the transport line alone, which SRS 1.129 says is weaker than it looks | FR-1230's generated card against an existing contact — no camera, no write until Save |
| ~~**SRS 1.193 — the undo line, all three branches**~~ | **PASS on all three.** `dropped=1` (8 Sep, offline so the account could not be touched), `deleted=1` (8 Sep, on a real write), `restored=1` (9 Sep, SRS 1.194, on a hand-made contact). **Still owed**: the **home screen** path, where an offer has outlived the process that made it | done |
...
person key yet — the key ships with this build. So the fixture has to be built: save a card, then
re-capture it. That is precisely the sequence that put two Arvind Bhandaris in the account on
5 September, which is what makes it the right test and why it **writes to the account**.

| Check | What failure looks like | Fixture |
|---|---|---|
| ~~**A re-photographed card is flagged before the save**~~ | **PASS, 6 Sep 2026.** A card saved, then photographed again — a fresh photograph, so a different source hash and nothing for FR-1208 to find. The sheet said *"You may already have this person: a contact with this name and mobile number is in your Google Contacts. Saving will add a second one."* and `Save contact` read **`enabled="true"`**. That pair *is* the requirement. It fired again on a **third** distinct photograph, so the key held across three readings rather than once. | Done |
| ~~**A card with no labelled mobile says nothing**~~ | **PASS, 6 Sep 2026, and a meaningful negative rather than a vacuous one.** The JSW card's only number is a switchboard typed `WORK`, so no person key forms — while the keyed contact sat in the account throughout, which is the condition that makes the silence worth observing. | Done |
| **The exact answer still outranks it** | Share the *same image file* twice, so the source hash matches: the sheet must say *"Already saved. Nothing was written again."* and **not** the weaker line beside it. **Not reached on 6 Sep**: every capture in that pass was a fresh photograph, and this row needs one file twice. The JVM test pins the branch | one image, shared twice |
| **B1 — the classifier** | **Built and tuned against eleven real cards, 5 Sep 2026 (SRS 1.109–1.117).** The first version scored **one name in five** — it took logotypes as people's names, and a logo is not identifiable by case. Eleven cards later: five rewrote every rule, four changed one or two rules each, and **two needed no change at all**. Rules cover logotypes, straplines, post-nominals (`NAME, IOFS`), single-letter phone labels, several values on one line, space-separated Indian PINs, companies told apart from corporate boilerplate, and — SRS 1.117 — the two places OCR breaks a field outright: a domain split after its dot, and a `+` parted from its country code. Both repairs fire only on input that is already invalid, and both are pinned by tests that try to make them invent. **FR-1222 stands at 52 of 120 — 68 owed by the developer** (SRS 1.207, 1.209; 11 until the 11 Sep 2026 scanning run). **Card twelve was the same card again** (SRS 1.126): the recogniser bled Devanagari into Latin — `U+0902` inside `Bhagat`, `U+0964` for a digit in the phone — and the mark alone cost the *name*, demoting it to the notes and promoting `Corporate Affairs` into the name field. A contact called "Corporate Affairs" is FR-1224's harm from one codepoint. Repaired only where Unicode forbids the sequence outright (a Devanagari combining mark on a Latin base), confined to the Devanagari block so an accented Latin name is untouched, and both guards confirmed to go red when the base check is removed. The danda is **left alone**: deleting it would give a more plausible wrong number than the truncation the user can see. The corpus **counts cards, not rows** now — a second recognition earns a row and not a count. | Ongoing, one card at a time |
| **B2 — the camera and its entry point** | **BUILT 5 Sep 2026 (SRS 1.119–1.121), JVM-verified and DEVICE-OWED.** FR-1201a through `ACTION_IMAGE_CAPTURE`: **no permission, no dependency, no `<queries>` entry, zero APK bytes** — the mechanism decision v1.85 left owed, taken and recorded. A filled **Capture a business card** button on Home, and the button is itself FR-1202's card-versus-date choice, so the photograph routes **straight to the card sheet** and nothing is ever detected into a mode change. `cardPhotoStep` waits for **both** the recognition and the QR decode before opening — the opposite ordering to SRS 1.100's, and the reason `cardPayloads` is tri-state now. `CaptureLayer.CAMERA` is added for FR-1207's provenance and is deliberately **not** an FR-1003 layer. FR-1211 is amended (SRS 1.120): the photograph is a file in a `camera/` cache subdirectory of its own — **not** FR-1005's `exports/`, which `shareAsIcs` clears in full — swept before each photograph and deleted when the capture window closes. **Watched on a device 5 Sep 2026** — see the rows below; six struck, and the pass found two things. | **Fourteen rows below, six struck** |
| **B3 — both sides in one capture (FR-1225)** | **BUILT 5 Sep 2026 (SRS 1.124), JVM-verified and DEVICE-OWED.** *Add another photo* on the card sheet; up to **four** photographs; the recognised lines concatenated in the order taken and classified **once**, never two drafts merged. Eight readings in SRS 1.124, three of them found by building it: the set is **enumerated from disk** rather than remembered, so a rotation costs a re-recognition instead of every side but the first; a new photograph **re-classifies the whole capture and keeps the user's edits**, which is the first use of `CardSheetState`'s `parsed`/`edits` split for anything but display; and **Save is refused while a photograph is being read**, because a save that raced the back would write a contact missing it, silently, against a card no longer in the hand. The cap is enforced by **withdrawing the control**, so FR-1225's "more added than read" line reports the case that can actually happen — a photograph that yielded no text. **Watched on a device 5 Sep 2026 — all nine rows struck**, and the pass found a defect that had been sitting in the lint report for a slice (SRS 1.125). | **Nine rows below, all struck** |

### Device pass backlog — B3, both sides in one capture (FR-1225)

**Nothing below has been done by a person.** The rows that matter most are the two that can fail
*silently*: a second side that does not reach the classifier, and a save that goes while one is
still being read. Both leave a plausible-looking contact and a card that is no longer in the hand.

| Check | What failure looks like | Fixture |
|---|---|---|
| ~~**A second side reaches the same contact**~~ | **PASS, 5 Sep 2026, several times.** `chars` **141 → 310** on the second photograph. One classification over the concatenated lines gave **three** phone numbers where the front alone gave one — the front's work line plus the back's Direct and Mobile, each typed correctly — the email from the front, the URL and the full address from the back, and the front's name, title and company kept. Two merged drafts could not have produced that set. | Done |
| ~~**The control is not offered on a decoded card**~~ | **PASS, 5 Sep 2026, on a fixture built to make the two paths tell themselves apart.** `testdata/fr1203/card-qr.png` prints one person's details and encodes a **different** person's in its vCard QR — printed `Vikram Chandrasekaran`, encoded `Anita Kapoor` — so which path ran is legible at a glance rather than inferred. Photographed off a screen, the sheet read **Anita Kapoor / Northwind Textiles Private Limited / Head of Sourcing** with the vCard's phone and email, and **Add another photo was absent**, as was FR-1224's "read from the photo" caption. **Two further confirmations came free.** `LatchCardOcr` printed **nothing at all** — `photoCardState` does that logging and was never called, so the classifier structurally did not run. And the **first attempt is the positive control**: shot from too far away the QR did not decode, the sheet showed `Southern Weaving Mills` and the printed phone, and **Add another photo was present** — so the control's absence is a consequence of decoding and not a thing that is simply never drawn. | Done |
| ~~**An edit survives another side**~~ | **PASS, 5 Sep 2026, in its strongest form.** Job title edited to `Executive Vice PresidentCORRECTED …`, then **three** further sides added. It was still there at the cap. The re-classification demonstrably ran each time — one phone became three — so this is not a sheet that failed to update, it is `parsed`/`edits` doing exactly the job it was written for. | Done |
| ~~**Save is off while a side is being read**~~ | **PASS on the state, 5 Sep 2026, with the button's own flag derived rather than read.** Caught mid-recognition: *"Reading the photo you just took. Save is off until it is in."* with the control reading **"Reading…"**. What was **not** captured in the same dump is `Save contact`'s `enabled` attribute — the window is under a second and the poll cycle is close to it. The note renders only when `blocker == READING_A_PHOTO` and `enabled` is `blocker == null` from the same value one expression away, so it follows; it is derived, and `CardSheetTest` is what pins it. | Done, with that caveat |
| ~~**The cap withdraws the control**~~ | **PASS, 5 Sep 2026, twice.** At four photographs **Add another photo is absent** from the action row on both runs, and no fifth shot was ever taken and discarded. | Done |
| ~~**A side that gives nothing is reported**~~ | **PASS, 5 Sep 2026.** Three blank surfaces added after a card front: the sheet read **"1 of 4 photos read. One gave Latch nothing — retake it if it matters."** and `chars` held at 141 through all four extractions rather than drifting. | Done |
| ~~**A rotation keeps every side**~~ | **PASS, 5 Sep 2026.** Forced to landscape and back with two sides in hand: `chars=310` **both ways**, never 141, and the sheet returned with both. That is the disk-enumeration reading of SRS 1.124 earning its place — remembered state would have kept the front and dropped the back. | Done |
| ~~**FR-1211 over a set**~~ | **PASS, 5 Sep 2026, on two files and again on four.** `card-1.jpg` … `card-4.jpg` present during the capture; after it closed the **directory itself is gone** and the cache is back to its single `data` entry. Also confirmed in passing that **adding a side does not sweep**: `card-1.jpg` was untouched on disk while the camera was open for the second. | Done |
| ~~**One contact, one undo**~~ | **PASS, 5 Sep 2026, at the second attempt — and the first attempt is why SRS 1.125 exists.** A two-sided card carrying three phones, an email and an address was saved: *"Saved to your contacts."* with **Undo (10)**, so Google returned a resource name and a contact was created. Undo inside the window gave **"Undone. Nothing was kept."** — the sentence that is rendered only where `removed == attempted`, so Google **accepted the delete** — and the window stayed open to say it. Device contact count **3174 before and 3174 after**, and no `Ramesh Kumar Bhagat` on the device at any point. **The count is corroboration and not proof, and the distinction is recorded rather than glossed**: the device's two existing `Bhagat` contacts were last touched on 3 Sep, so nothing establishes that this phone syncs a People API creation within the observation window — a flat count is consistent with the undo working and would also be consistent with a slow sync. What carries the row is the sheet's own sentence, which is only evidence *because* SRS 1.125 gave the failed outcome a different one. On the **first** attempt none of this was readable: the answer was discarded and the window simply closed. | Done, with the count as corroboration |

**A note on `testdata/fr1203/card-qr.png`.** It is **generated, not a real card**, and it must never
enter FR-1222's corpus for that reason — the corpus counts cards somebody was actually handed, and
this one was made by `MakeCard.java` from the vCard grammar using the ZXing already on the
project's classpath (no new dependency). Its whole value is that the printed identity and the
encoded identity **disagree on purpose**, which is what makes "did the grammar answer, or the
classifier?" a question a screenshot settles. The device row it exists for is FR-1203's, not
FR-1222's. It replaces `card-anita.png`, which the Phase A rows name and which was never
committed.
...
~~**Phase A ships the shared image only.**~~ **The camera shipped at B2, 5 Sep 2026, and the
decision is `ACTION_IMAGE_CAPTURE`** (SRS 1.119) — no permission, no dependency, no `<queries>`
entry, and therefore no APK figure to weigh under NFR-501 rather than a small one. CameraX is
reopened **only on evidence**: a device pass showing a high retake rate is what would make a
framing guide worth a dependency and the `CAMERA` runtime permission. Taste is not that evidence,
and the retake rate is the first thing to watch when the button is finally pressed.

### Device pass backlog — B2, the camera (FR-1201a, FR-1202, FR-1211)

**Nothing below has been done by a person.** Read each row as *the condition that would make it
fail*, then *the fixture*. The FR-1211 rows matter most: they are the only place a third party's
photograph is governed, and a file that quietly outlives its capture looks exactly like one that
does not.

| Check | What failure looks like | Fixture |
|---|---|---|
| ~~**The button opens a camera at all**~~ | **PASS, 5 Sep 2026.** Google Camera's `CaptureActivity` on top, in Latch's own task, with **no permission prompt** — and the installed APK requests no `CAMERA` at all, read off `dumpsys package` rather than off the source. The requested list is exactly the seven the manifest documents. | Done |
| ~~**The photograph reaches the card sheet and nothing else**~~ | **PASS, 5 Sep 2026, three times.** The card sheet opened directly every time — never a dates sheet with a "Save as contact" button beside it. That difference *is* FR-1202 on this path: the choice was made at the button. | Done |
| **The spinner names the card** | "Reading the card…", not "Reading the text…". Not watched on 5 Sep: the recognitions ran 416–1019 ms and the spinner was never screenshotted. A one-word check, and the only thing separating a wait the user understands from one that reads as the wrong screen having opened | any card |
| **Backing out of the camera costs nothing** | Press the shutter, then discard in the camera application; and separately, back out before shooting. Latch must show **nothing new** — no empty capture sheet, no card sheet — and `ls` under the app's cache must show no photograph. Failure is a stranded window or a photograph left on disk | the camera's own discard |
| ~~**FR-1211 — the photograph does not outlive the capture**~~ | **PASS, 5 Sep 2026, and it is the row the whole FR-1211 amendment rests on.** During the capture: `cache/camera/card.jpg`, 1,680,685 bytes. After the sheet was closed: **the directory itself is gone**, and `cache` is back to the single `data` entry it held before any photograph. Watched rather than reasoned about, which for a third party's photograph is the difference that matters. | Done |
| **FR-1211 — a rotation does not lose the capture** | Rotate the phone while the card sheet is up. The sheet comes back with the same fields. Failure is an empty sheet or an unreadable-image message, which would mean the file had been deleted at first recognition after all — the stricter rule this one was chosen over. **Re-run under B3**, where the stakes are higher: a rotation must bring back *every* side, not just the first | a card, and a quarter-turn |
| ~~**FR-1211 — a killed process leaves at most one, and the next photograph takes it**~~ | **PASS, 5 Sep 2026, on a fixture better than the one written for it.** A reinstall killed the process with a card sheet open, leaving `card.jpg` at 1,662,133 bytes on disk — which is the precondition this row needs and could not have been faked. The next photograph, of a different card, left exactly one file at 1,746,958 bytes. One file, and it is the new one. | Done |
| **A card carrying a QR opens from the code, not the photograph** | Photograph `card-anita.png` on a screen, or any card with a contact QR printed on it: the sheet must be the **grammar's** reading — every field exactly right — not the classifier's. Failure is a plausible but slightly-wrong sheet, which is the race `cardPhotoStep` waits out and is invisible unless the two readings are compared | a card with a contact QR |
...
**The count after reconciling was 81 open of 108, and an automated read-only pass on 10 Sep closed seven more: 74 open of 108**, and the shape of what is left is more useful
...
| ~~A short PDF still says the same thing~~ **PASS 10 Sep 2026 — `letter.pdf` showed `Reading page 1 of 2…`, the cap line correctly absent, `pages=2/2`, `TASK 14 Oct 2027`.** Original row: | **PARTLY: the cap line is correctly absent for `letter.pdf` (31 Aug). The 1→2 **progress line** is not watched.** `letter.pdf` (2 pages) counts 1→2 and shows **no** cap line | `testdata/fr215/letter.pdf` |
| ~~An **image** capture is unchanged~~ **PASS on the half that matters, 10 Sep 2026 — **zero** occurrences of "Reading page" across ~44 samples on two fixtures including a 2800x2000 photograph. **The positive was not caught**: `capture_extracting` is "Reading the text…" and would have matched the same grep, but the image path finishes in ~1.7 s against ~0.7 s per `uiautomator dump`, so it was never sampled.** Original row: | The spinner says "Reading the text…", never "Reading page…". An image has nothing to count, and a progress line on one would be a number invented from nothing | any `testdata/fr215/*.png` |
...
| ~~**FR-807 across a process death**~~ **PASS 10 Sep 2026 — `am force-stop` two seconds after the save, and the home screen came back with **"Just saved. You can still undo it." / Undo (6)**. This is the limit FR-807's own note recorded as unfixable without FR-701.** Original row: | Save, force-stop inside the ten seconds, reopen: the home screen offers Undo with the remaining seconds, and taking it removes the item from Google. This is the limit FR-807's note recorded as unfixable without FR-701 | "Kickoff 8 September 2027 at 9am" |
| ~~**FR-807 across a rotation**~~ **PASS 10 Sep 2026 — saved, rotated to landscape, and the offer was still standing (`Saved to Latch | Undo (5)`); the undo then removed the item, `deleted=1 failed=0`.** Original row: | Save, rotate inside the ten seconds: the offer is **still there**. Before this slice the recreation reset the saver and the offer vanished — a latent defect no test could see | any capture, rotate |
...
| ~~FR-806's immediate drain~~ **PASS 10 Sep 2026 — the network restored, `Worker result SUCCESS for WriteQueueWorker` and the queue back to `<map />` within about 35 seconds, not at the next backoff.** Original row: | Queue a capture offline, wait past one backoff, turn the network on: the drain runs within seconds rather than at the next scheduled attempt | aeroplane mode on → capture → off |
| ~~FR-806's Retry now~~ **PASS 10 Sep 2026 — offered on the home screen beside *1 capture waiting to be saved to Google*, and **tapped while still offline it left the entry queued and lost nothing**.** Original row: | With something queued, the button appears and a tap drains. With a given-up entry, the tap revives it and it is attempted again | a 403 or an entry given up on |
...
| ~~An override reaches Google~~ **PASS 10 Sep 2026 — flipped to TASK, `21 Sept 2027` with the time dropped, written as `Create basis=NO_MATCH`.** Original row: | Overridden to TASK, the save produces a **task due 8 Sep 2027** and no event | as above |
| ~~A range overridden to a task~~ **PASS 10 Sep 2026, both halves — the sheet showed `20 Sept 2027 – 24 Sept 2027` then `20 Sept 2027` after the flip, and it was written.** Original row: | **PARTLY: the sheet half passed 7 Sep (dropped to the first day). **The written task has not been checked in Google.**** The note names the lost end date, and the saved task is due on the **first** day | "Trip from 20 September to 24 September 2027" |
| ~~Per-row override in a chain~~ **PASS 10 Sep 2026 — two rows, the second flipped, one event and one task written.** Original row: | Two rows, one flipped: the save writes one event and one task | a two-date capture |
...
| ~~FR-510 per candidate~~ **PASS 10 Sep 2026 — `12 Mar 2026` (the current year, never rolled forward) carrying the past note and the fixed badge, beside `20 Sept 2027` carrying neither; both written.** Original row: | **PARTLY: two ticked rows with the right notes passed 7 Sep. **The two items reaching Google are not verified.**** "Invoice dated 12 March, payment due 20 September 2027" → **two** items: an undated follow-up and a task due 20 Sep | as quoted |
...
| Check | What failure looks like | Fixture |
|---|---|---|
| ~~**AC-06**~~ **PASS on Android 10 Sep 2026, re-based to `Project sync on 14 September 2027 at 11:00` so the fixture lands in one month. Tue 14 Sept → three working days back → **Thu 9 Sept 2027**, and FR-606 read **"weekend or rest days skipped (2 days)"** — the count corroborating the date rather than restating it. **This had only ever run on Windows**.** Original row: | "Project sync on 8 September 2026 at 11:00" + *Meeting + prep* → the prep task lands on **Thu 3 Sep**, and the row says the weekend was skipped. 8 Sep 2026 is a Tuesday, which is what makes the weekend get crossed | as quoted |
| ~~The chain reaches Google intact~~ **WRITTEN 10 Sep 2026 — three items from one capture. **The account-side confirmation is the developer's**, in the log handed over with this run.** Original row: | Three items — one event, two tasks — all in the Latch calendar and the chosen list, sharing one `latch.chain_id`, and `latch.recipe` reading `builtin.meeting_prep`. **Inspect the written items**, not the screen | as above |
...
| ~~The chooser is absent for a multi-date capture~~ **PASS 10 Sep 2026 — a two-date capture showed *A recipe expands one date, and this capture has several* and no chooser.** Original row: | A four-date capture shows the reason, not a chooser | any multi-date capture |
...
| Check | What failure looks like | Fixture |
|---|---|---|
| ~~Settings opens and lists real calendars and task lists~~ **PASS 10 Sep 2026 — three writable calendars including Latch, and the task list, all populated from the account.** Original row: | Both lists populate; the current destination is selected; a hidden calendar shows its badge (FR-903) | a real account |
...
| ~~FR-605 from Settings~~ **PASS 10 Sep 2026 — the same run, from the Settings side: a stored preference changing the *arithmetic*, not the rendering.** Original row: | Change the working week to six days; AC-06's prep step moves from Thu 3 Sep to Fri 4 Sep | AC-06's capture |
| ~~**FR-904's picker**~~ **PASS 10 Sep 2026 — *Change* expands the calendar list **inline without blocking the sheet**, all three writable calendars are offered (the third is below the fold), and choosing one closes the list and moves the chip. **The write half is not claimed**: it would put a fixture in a real working calendar, so it was not done.** Original row: | Tap Change on the sheet: the calendar list loads **and the sheet does not block while it does**. Choosing one changes the chip, and the write goes there | any capture |
...
| ~~**FR-1005**~~ **PASS on the file, 10 Sep 2026 — the half a device can settle without another app. **The "it opens as a calendar file" half needs a person** and stays owed.** Original row: | Export .ics from the sheet and share it to Google Calendar or a mail client: the file **opens as a calendar file** and imports the right dates. A file nothing will open is the failure | "Kickoff 8 September 2027 at 9am" |
...
| ~~Two exports do not collide~~ **PASS 10 Sep 2026 — the same capture exported twice minted `ab181b76…` then `62131d41…`, so a reader importing both gets two items rather than one overwriting the other.** Original row: | Export the same capture twice and import both: **two** items, not one overwriting the other | as above |
| ~~A Devanagari title exports readably~~ **PASS 10 Sep 2026, measured in octets rather than eyeballed.** Original row: | Export a Hindi capture and open the file: the title is intact, not mojibake. This is the folding case no JVM test can fully close | a Devanagari capture |
| ~~An export before saving~~ **PASS 10 Sep 2026 — no `save decision=` line and `latch.db` mtime unchanged, so nothing reached Google and nothing was written locally either.** Original row: | Export without pressing Save: **nothing** is written to Google | any capture |
