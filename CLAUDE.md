...
| **5b — reversal** | **PART BUILT 4 Sep 2026 (SRS 1.99).** FR-1210's undo is built and tested: a **written** contact is undone by deleting it, a **queued** one by dropping the entry — and `cardDrainable` keeps the drain out of the undo window, the same window governing both. FR-1212's `drainCard` re-runs FR-1208 per entry, keeps the entry when a search or a create throws, and writes the **captured** instant rather than the drained one. **NOT built, so FR-1212 is NOT met: the persistent store and the worker wiring.** `CardSaver` reports a failure rather than enqueueing, deliberately — accepting a card and never writing it is worse than declining it | Owed with the store |
