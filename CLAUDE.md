...
| Check | What failure looks like | Fixture |
|---|---|---|
| **A re-photographed card is flagged before the save** | Save a card with a name and a labelled mobile. Photograph it again: the sheet must say *"You may already have this person…"* — **and Save must still work**. Failure in the dangerous direction is Save being disabled or the capture refused; failure in the useless direction is silence, which is what happens today | any card with an `m`-labelled mobile — **writes to the account** |
| **A card with no labelled mobile says nothing** | The JSW card, whose only number is a switchboard. No warning either time, however many times it is captured. This is the degenerate case the key refuses to exist for, and silence is the correct behaviour rather than a bug | `jsw-steel`'s card |
| **The exact answer still outranks it** | Share the *same image file* twice, so the source hash matches: the sheet must say *"Already saved. Nothing was written again."* and **not** the weaker line beside it | one image, shared twice |
