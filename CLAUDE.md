...
| **Dismissing the card sheet** (SRS 1.104) | Save a card from an image with **no dates** in it, then dismiss: the **whole capture closes**. Then a capture holding both a code and a date: after saving the contact, dismissing returns to the **dates**, which are still unsaved. Failure is being dropped onto an empty capture sheet after finishing, which is what it did before | `card-anita.png`; and a flyer with a date and a code |
