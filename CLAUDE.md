...
| **The Inbox opens and triages** | **Four of five done, 4 Sep 2026.** Tray read *1 waiting*, the entry opened the window, the row stated its reason and offered the chips without a press. **Assign a date**, **save**, **edit** (title change survived a close-and-reopen) and **snooze** (left both the list and the count) all done. **Still owed: discard.** | any Inbox row |
...
| **A held row survives a restart** | **Half done, 4 Sep 2026.** After a quit-and-restart the record was **observed on disk** — `inbox.dat`, one row, encrypted, with the captured text not in the clear — so the store survives a process boundary, which its own tests cannot show because they write and read in one process. **The visible half is owed**: the row had been snoozed first (a sequencing mistake in the pass, not a defect), so it is correctly hidden from the list until 11 Sep. Needs one fresh unsnoozed row and a restart. | any undated capture |
