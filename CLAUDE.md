...
| **B1 — the classifier** | **BUILT 4 Sep 2026 (SRS 1.107), device pass OWED.** `classifyCard` in `:cards`: guesses narrowly and shows what is left. A job title comes from a closed vocabulary and **not** from "the line after the name"; an organisation only from a real suffix; a phone needs seven digits so a postcode cannot become one. Every line reaches a field or the unplaced list — asserted, so nothing is silently consumed. FR-1224's notice sits **above** the fields. **FR-1222's corpus is empty and owed: 0 of 120 real cards, which only the developer can supply.** | **Owed** |
