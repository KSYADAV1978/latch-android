...
| **4 — decode and sheet** | **PART BUILT 4 Sep 2026 (SRS 1.96).** ZXing adopted behind `QrReader` in `:ocr` at a measured **+17,176 B**, no native code, `DEPENDENCIES.md` entry written (closes FR-1240(b)). Decode runs **off** the critical path so NFR-101's 2.5 s is spent on the dates; the card offer folds in after. FR-1202's `cardOffer` and FR-1205's `CardEdits` are pure and tested — no value means *switched*. **OWED: the Compose preview, FR-1213's account line, and the "Save as contact" action.** | — |
