...
| **The menu dismisses** | **FAILED in real use, 9 Sep 2026, exactly as this row predicted; fixed at SRS 1.196 and NOT yet re-watched.** The invoker was a `JWindow`, which `Window.isFocusableWindow()` refuses twice over, so `requestFocus()` was inert and `windowLostFocus` could never fire. It is a `JDialog` now. **Re-run both halves**: click away from the menu — it closes; press Esc — it closes. Esc failed for the same reason and has never been pressed | the tray, on a build at or after SRS 1.196 |
