...
| **FR-1212 — a card held offline reaches the account** | Aeroplane mode, share a card QR, Save. Expect **"No connection. This card is held on your phone"** — never "saved", which is the distinction the Held state exists for. Turn the network on: the drain writes it. Failure is a card that is accepted and never arrives | any unsaved card QR, offline |
| **FR-1212 — a held card survives a restart** | Hold one offline, **force-stop Latch**, reopen, reconnect: it still drains. The record is on disk under the Keystore, and a JVM test cannot show that it survives a real process | as above, plus `am force-stop` |
| **FR-1212 — the check runs again at drain** | Save a card online. Then aeroplane mode, capture the **same** card, Save — it is held. Reconnect. Expect the entry to **retire without writing**, and the account to hold **one** contact, not two. This is AC-07 failing inside AC-10 on a third transport, and it is the row the calendar's equivalent failed | one card, saved then re-captured offline |
| **FR-1210 — undo of a held card deletes nothing** | Hold a card offline, then Undo inside ten seconds. The entry goes and **nothing is ever written** on reconnection. Failure is a contact appearing minutes later, having been "undone" | any card, offline |
...
| ~~**5b — reversal**~~ | **BUILT 4 Sep 2026 (SRS 1.99, 1.105), device pass OWED.** FR-1210's undo: a written contact is deleted, a **held** one has its entry dropped. FR-1212's store is its own — `card_queue`, own Keystore alias, JSON record holding the confirmed draft **and** the payload, which answer different questions. `CardSaver` holds on both failure paths; a saver with no queue still reports a failure. The drain runs on FR-806's worker, skips entries inside their undo window **without consuming an attempt**, keeps a given-up entry rather than deleting it, and does not let one failure hold up the entries behind it. | **Owed** |
