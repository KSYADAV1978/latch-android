...
| **B1 — the classifier** | **REWRITTEN against five real cards, 5 Sep 2026 (SRS 1.109).** The first version scored **one name in five**: it took logotypes — `TALS`, `Deloitte.`, `informa` — as people's names, and the logo is not identifiable by case. A name is now two to four words of letters. Also fixed from real cards: a space before the `@` (which lost an address and its FR-1209 identity), single-letter phone labels, two values on one line, two numbers on one line, a company split across lines, a suffix rule that matched *"A Mini Ratna Company"*, and no address rule at all. **FR-1222 stands at 5 of 120 — 115 still owed by the developer.** | Ongoing, one card at a time |
