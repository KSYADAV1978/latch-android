...
| Check | What failure looks like | Fixture |
|---|---|---|
| **A second side reaches the same contact** | Photograph the front, press **Add another photo**, photograph the back. The sheet must gain the back's fields — an address, usually — **while keeping the front's name**. Failure is the sheet showing only the back, which is what "the last photograph wins" looks like and is a perfectly plausible draft | a two-sided card |
...
| **An edit survives another side** | Correct the job title, *then* add the back. The correction must still be there. Failure is silent and total: `CardSheetState`'s `parsed`/`edits` split is the only thing preventing it, and this is the first thing that has ever used it | a two-sided card |
| **Save is off while a side is being read** | Press Add another photo, take it, and watch the moment it returns: Save must be disabled with *"Reading the photo you just took"* until the fields update. Failure writes a contact missing the back — silently, and against a card the user has put away | a two-sided card |
| **The cap withdraws the control** | Add four photographs. **Add another photo** must be gone at the fourth, and no fifth photograph is ever taken. Failure is a fifth shot accepted and discarded, which is worse than a button that stops | a card, photographed four times |
| **A side that gives nothing is reported** | Add a photograph of a blank surface as the second side: the sheet must say **"1 of 2 photos read"** and keep the front's fields. This is FR-1225's report and the only case it can arise from | a card, then a blank wall |
| **A rotation keeps every side** | Photograph both sides, then rotate. **Both** must come back. This is the row the disk-enumeration reading was taken for; remembered state would have kept the front and dropped the back, which loses work rather than repeating it | a two-sided card, and a quarter-turn |
| **FR-1211 over a set** | With two sides taken, `cache/camera` holds **two** files. Close the capture: the directory is gone. The single-photograph version of this passed on 5 Sep; the set has never been watched | `adb shell run-as com.latch.android ls -l cache/camera` |
| **One contact, one undo** | Save a two-sided card: **one** contact in the account carrying both sides' fields, and one Undo removes it. FR-1225's "one capture, one classification, one duplicate check, one undo" is four claims and this row is the last two | a two-sided card — **writes to the account** |
