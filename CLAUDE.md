...
| Check | What failure looks like | Fixture |
|---|---|---|
| **The button opens a camera at all** | Home shows a filled **Capture a business card** above Inbox/Recipes/Settings; pressing it opens the phone's camera. Failure is the no-camera sentence appearing on a phone that plainly has one, which would mean `ACTION_IMAGE_CAPTURE` resolved to nothing — the case the `<queries>` entry was deliberately not added for | any card |
| **The photograph reaches the card sheet and nothing else** | After the shutter and the camera's own confirm, Latch opens **on the card sheet**, not on a dates sheet with a "Save as contact" button beside it. That difference *is* FR-1202 on this path: the choice was made at the button | a printed card |
| **The spinner names the card** | "Reading the card…", not "Reading the text…". A one-word check, and the only thing separating a wait the user understands from one that reads as the wrong screen having opened | any card |
| **Backing out of the camera costs nothing** | Press the shutter, then discard in the camera application; and separately, back out before shooting. Latch must show **nothing new** — no empty capture sheet, no card sheet — and `ls` under the app's cache must show no `camera/card.jpg`. Failure is a stranded window or a photograph left on disk | the camera's own discard |
| **FR-1211 — the photograph does not outlive the capture** | Take one, let the sheet open, close the capture. Then `run-as com.latch.android ls cache/camera` — **no such file**. This is the row the FR-1211 amendment rests on and the only one that can show it | `adb shell run-as com.latch.android ls -l cache/camera` |
| **FR-1211 — a rotation does not lose the capture** | Rotate the phone while the card sheet is up. The sheet comes back with the same fields. Failure is an empty sheet or an unreadable-image message, which would mean the file had been deleted at first recognition after all — the stricter rule this one was chosen over | a card, and a quarter-turn |
| **FR-1211 — a killed process leaves at most one, and the next photograph takes it** | Photograph a card, `am force-stop`, confirm `cache/camera/card.jpg` is **still there** (this is the sweep's own precondition — a row that cannot show the file first proves nothing), then photograph a second card and confirm the file's content is the **new** one. Failure is two files, or the first one surviving | `adb shell am force-stop com.latch.android` |
...
| **NFR-101 for text is unmoved** | An ordinary text capture still reaches a filled sheet under 800 ms, `content ready, ocr=false` and no spinner. The standing re-check whenever `CaptureActivity` is touched | "Kickoff 8 September 2027 at 9am" |
| **The retake rate** | Not pass/fail. Count how many cards need a second photograph across the first dozen. That number is the only thing that reopens CameraX, and it is worth writing down while the impression is fresh rather than recalled later | the first dozen real cards |
