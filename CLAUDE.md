...
| **B1 — the classifier** | **Built and tuned against ten real cards, 5 Sep 2026 (SRS 1.109–1.116).** The first version scored **one name in five** — it took logotypes as people's names, and a logo is not identifiable by case. Ten cards later: five rewrote every rule, three changed one rule each, and **two needed no change at all**. Rules now cover logotypes, straplines, post-nominals (`NAME, IOFS`), single-letter phone labels, several values on one line, space-separated Indian PINs, and companies told apart from corporate boilerplate. **FR-1222 stands at 10 of 120 — 110 owed by the developer.** | Ongoing, one card at a time |
